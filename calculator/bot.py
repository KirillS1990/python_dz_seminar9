...
bot = TeleBot("5723009710:AAEx_f47TN_9wvfS_h9qjZ0TX0kYOPZwwuM")
...
def start(msg):
    bot.send_message(chat_id=msg.from_user.id, text=f"Добро пожаловать в бот - калькулятор")
    bot.send_message(chat_id=msg.from_user.id, text=f"Для того чтобы посмотреть возможности программы введите команду /help")
    bot.send_message(chat_id=msg.from_user.id, text=f"Введите команду или первое число")
...
def help(msg):
    bot.send_message(chat_id=msg.from_user.id, text=f"Программа Калькулятор.\n"
                                                    ...
                                                    f"Комплексное число нужно записывать в формате n+ki (например 5-2i)\n"
...
def calculations(msg: telebot.types.Message):
    ...
    if check_number(msg.text) != "Ошибка":
        ...
        bot.send_message(chat_id=msg.from_user.id, text=f"Результат операции {print_number(number)}")
        bot.send_message(chat_id=msg.from_user.id, text=f"Введите операцию которую хотите произвести или новое число")
