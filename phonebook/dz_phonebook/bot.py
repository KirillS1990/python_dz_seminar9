...
def main(message):
    if message.text == '/main':
        ...
        bot.send_message(
            message.chat.id, f'Введите слово "download", для того чтобы скачать файл с контактами')
        bot.register_next_step_handler(message, download)

    elif message.text == '/9':
        lg.logging.info('The user has selected item number 9')
        bot.send_message(
            message.chat.id, f'Прикрепите файл с контактами для импорта')
